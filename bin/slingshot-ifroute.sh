...
Arguments:
  INTERFACE   Name of a single network interface (e.g. hsn0, hsn12).
...
# ALL_HSNS - all HSN interfaces present in the system
ALL_HSNS=$(ls "${NET_DIR}" | grep '^'"${DEV_PREFIX}"'[0-9]\+$')
...
if [[ -z "$ARG_INTERFACE" ]]; then
    ...
    if [[ ! "$ARG_INTERFACE" =~ ^hsn ]]; then
        ...
        VALIDATION_FAILED=1
    elif ! echo "$ALL_HSNS" | grep -qx "$ARG_INTERFACE"; then
...
for device in ${INTERFACES} ; do
    ...
    found=$(awk -v label="$label" '$2 == label {count++} END {print count+0}' "$RT_TABLES")
    unit=${device#${DEV_PREFIX}}
    let index=200+${unit}

    if [[ ${found} -eq 1 ]] ; then
        echo "${label} already exists: $(awk -v label="$label" '$2 == label' "$RT_TABLES")"
    else
        if [[ ${found} -eq 0 ]] ; then
